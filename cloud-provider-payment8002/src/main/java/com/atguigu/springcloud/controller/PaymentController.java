...
public class PaymentController {
    ...
    public CommonResult create(@RequestBody Payment payment) {
        ...
        if (result > 0) {
            return new CommonResult(200, "添加成功" + serverPort, payment.getId()); // 显示新添加的ID而不是改变行数。
    ...
    public CommonResult getPaymentById(@PathVariable("id") Long id) {
        ...
        log.info("*****" + serverPort + "查询结果：" + result);

        if (result != null) {
            return new CommonResult(200, "查询成功" + serverPort, result);
        } else {
            return new CommonResult(444, "查询" + id + "没有找到结果" + serverPort);
        }
